...
class NonTerminal(Symbol):
    ...
    def __imod__(self, other):
     ...
     #Esto se agrego para dar la cadena de conflicto
    
    #Esto se agrego para dar la cadena de conflicto
    def expand(self, marked=None, term=None):
        for p in self.productions:
            if (term and p.Right[0]==term) or not p.Right.IsEpsilon:
                try:
                    marked[p]
                    continue
                except:
                    for idx, r in enumerate(p.Right):
                        if r.IsNonTerminal:
                            marked[p]=True
                            resp= ''.join(''.join([str(el) for el in p.Right[:idx]])+r.expand(marked=marked)+''.join([el.expand(marked=marked) for el in p.Right[idx+1:]]))
                            return resp
                    resp=''.join(str(p.Right))
                    return resp
        return ''
...
class Sentence(object):
    ...
    def expand(self, marked=None, term=None):
        ...
        if len(self._symbols)>1:
            resp= ''.join([self._symbols[0].expand(marked=marked, term=term)]+[item.expand(marked=marked) for item in self._symbols[1:]])
            return resp        
        resp= ''.join([self._symbols[0].expand(marked=marked, term=term)])
        return resp
...
class Grammar():
    ...
    def from_json(data):
        ...
        G.startSymbol = G.Productions[0].Left
        return G
